...
export async function initDatabase() {
  ...
  initPromise = (async () => {
    ...
    try {
      ...
      `);
      // Migração: adicionar colunas de investimento se ainda não existirem
      try { await db.execute('ALTER TABLE companies ADD COLUMN principal REAL'); } catch (e) {}
      try { await db.execute('ALTER TABLE companies ADD COLUMN monthly_rate REAL'); } catch (e) {}
      // Migração: renomear Tesouro Direto → Rendimentos
      try { await db.execute("UPDATE companies SET name = 'Rendimentos' WHERE name = 'Tesouro Direto'"); } catch (e) {}
...
async function seedDatabase() {
  const companies = [
    ...
    { name: 'XP Inc', type: 'CLT', salary_monthly: 5800, start_date: '2026-01-01' },
    { name: 'Itaú', type: 'CLT', salary_monthly: 5100, start_date: '2026-01-01' },
    { name: 'Genial Investimentos', type: 'PJ', hourly_rate: 59.52, start_date: '2026-01-01', active: false },
    { name: 'nstech', type: 'CLT', salary_monthly: 6000, start_date: '2026-08-07' },
    ...
    { name: 'Founday', type: 'CLT', salary_monthly: 10000, start_date: '2026-02-23', active: false },
    { name: 'Ayn Rand Institute', type: 'PJ', hourly_rate: 80, start_date: '2026-04-15', active: false },
    { name: 'Taproot', type: 'PJ', hourly_rate: 100, start_date: '2026-04-15' },
    { name: 'Rendimentos', type: 'CLT', salary_monthly: null, start_date: '2026-08-01', principal: 550000, monthly_rate: 0.01 },
    { name: 'B3', type: 'CLT', salary_monthly: 6342.13, start_date: '2026-08-01' },
    { name: 'DM Financeira', type: 'CLT', salary_monthly: 12000, start_date: '2026-08-01' }
  ];

  for (const company of companies) {
    await db.execute({
      sql: `INSERT INTO companies (name, type, salary_monthly, hourly_rate, start_date, active, principal, monthly_rate)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      ...
      args: [
        ...
        company.principal ?? null,
        company.monthly_rate ?? null
      ]
    });
  }
