...
export async function initDatabase() {
  ...
  initPromise = (async () => {
    try {
      ...
      console.log('📝 Criando cliente Turso com URL:', process.env.TURSO_CONNECTION_URL.substring(0, 50) + '...');
      ...
      console.log('✅ Cliente Turso criado');

      // Testando conexão básica
      console.log('🔄 Testando conexão...');
      const testResult = await db.execute('SELECT 1 as test');
      console.log('✅ Conexão com Turso bem-sucedida:', testResult);
...
export async function getDatabaseAsync() {
  ...
  if (!db) {
    throw new Error('Database initialization failed - no db instance');
  }
  return db;
}
